...
class PdfExtractor
{
	constructor(outputDir, options) {
		options = options || {};
		options.canvasZoom = options.canvasZoom || 1.5;

		this.canvasZoom = options.canvasZoom;
		...
		this.renderers.push(new JsonRenderer(outputDir, options));
	...
	renderMetaData(doc) {
		return doc.getMetadata().then((metaData) => {
			...
			console.log(this.metaDataHandler);
		});
	}

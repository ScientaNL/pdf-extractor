...
class JsonRenderer extends AbstractRenderer
{
	...
	getPageFonts() {
		...
		for (const [fontName, font] of Object.entries(this.metaDataHandler.docFonts)) {
			fonts[fontName] = Object.assign({}, font);
			fonts[fontName].pages = [];
		}

		for (const [key, page] of Object.entries(this.metaDataHandler.pages)) {
			for (const font of page.fonts) {
				if (fonts[font.pdfFontName]) {
					fonts[font.pdfFontName].pages.push(page.pageNumber);
				}
			}
		}

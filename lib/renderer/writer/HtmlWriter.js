...
class HtmlWriter extends FileWriter
{
	...
	processTextStyles(page, styles)
	{
		for (const [fontName, style] of Object.entries(styles)) {
			this.metaDataHandler.addDocFont(fontName, style.fontFamily, page.pageNumber);
		}
	...
	changeTextLayerFont(styles) {
		...
		for (const [key, style] of Object.entries(styles)) {
			let pos = replacements.indexOf(style.fontFamily);
			if (pos > -1) {
				let replaceFont = this.fontFamilyReplacements[style.fontFamily];
				styles[key].fontFamily = replaceFont.fontFamily;
				if (this.metaDataHandler.docFonts[key]) {
					this.metaDataHandler.docFonts[key].setHtmlFontFamily(replaceFont.fontName);
				}
			}
		}
	}
